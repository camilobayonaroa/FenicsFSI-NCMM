...
def solve_move():
	...
	while Coup_L2_abs > tol and Coup_it < Coup_maxiter:
		...
		#Displace Solid's domain
		if processID == 0: print('\n     Solving STRUCTURE displacement...', '\n')

                front_mesh.rotate(float(theta0*sin(2*pi*frecrot*float(dt))),2)
                front_mesh.translate(Point(0.0,float(amp0*sin(2*pi*frecrot*float(dt)+phase))))
