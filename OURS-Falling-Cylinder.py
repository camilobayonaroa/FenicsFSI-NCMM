...
mass_s = rho_s*pi*Radius*Radius
nu_s = 0.4      # Poisson ration, in 1
mu_s = 1e6    	# in ton/(ms^2), original value 2e3
...
class ImposedCondition(UserExpression):
    def __init__(self, param, **kwargs):
        self.param = param
    def eval(self, values, RelativeVel):
        values[0] = self.param
        values[1] = 0.0
        values[2] = 0.0
    def value_shape(self):
...
ic=ImposedCondition(param= 0.1,degree =2)
...
# Inflow profile
#U_ini = 0.0
#inflow_profile = Expression(('U_rel','0.0'),U_rel=U_ini, degree=3)
...
def solve_move():
    ...
    bc_wal = MultiMeshDirichletBC(MultiMeshSubSpace(W,1), Constant(0.), chanfunc, 2, 0)
    ...
    w_rel = MultiMeshFunction(W)
    w_rel.assign(interpolate(ic,W))
    ...
    InitialTransient = 0.1 	# Until the pressure stabilizes, don't solve the solid	
	...
	if processID == 0: print('SOLVING STEP:', count, ', AT TIME: ', t, '\n')
        ...
        #fluid's velocity (relative to cylinder motion)
        if t > InitialTransient: 
               tsim += float(dt)
               U_act = U_ini + (grav-drag/mass_s)*float(dt) 
               U_ini = U_act
               inflow_profile.U_rel = U_act 
               ic.param= U_act
               w_rel.assign(interpolate(ic,W))

        wk.assign(project(wk+w_rel,W))
