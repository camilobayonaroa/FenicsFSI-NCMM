...
beta = Constant(1e1)    #Working with 1.0 
#VMS
SC1 = 12.0
SC2 = 4.0
SC3 = 1.0
...
beam = Rectangle(Point(Center_x,Center_y-0.5*w_beam), Point(Center_x+l_beam,Center_y+0.5*w_beam))
front_domain = circle + beam
back_mesh = generate_mesh(rectangle, resolution_back)
front_mesh = generate_mesh(front_domain, resolution_front)
...
class Cylinder(SubDomain):
   def inside(self, x, on_boundary):
      tol = 1e-3
      return on_boundary and (pow((x[0]-Center_x),2) + pow((x[1]-Center_y),2) ) <= pow((Radius+tol),2) 
...
def solve_move():
    ...
    dirfunc = MeshFunction('size_t', front_mesh, front_mesh.topology().dim()-1)
    dirfunc.set_all(0)
    dirbound.mark(dirfunc,1)
    ...
    bc_s = []
    bc_s.append( DirichletBC(V1, Constant((0, 0)), dirfunc, 1) )
